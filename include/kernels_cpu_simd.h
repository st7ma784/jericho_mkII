 ...
 */
void advance_particles_cpu_simd(
    double* x, double* y,
    double* vx, double* vy,
    ...
    int species_id)
{
    ...
    #pragma omp parallel for schedule(static) \
            aligned(x, y, vx, vy, Ex, Ey, Bz: CACHE_LINE_SIZE) \
            simd collapse(1)
    for (int p = 0; p < n_particles; p++) {
        ...
        double Ex_interp = w00 * Ex[idx00] + w10 * Ex[idx10] +
                          w01 * Ex[idx01] + w11 * Ex[idx11];
        
        double Ey_interp = w00 * Ey[idx00] + w10 * Ey[idx10] +
                          w01 * Ey[idx01] + w11 * Ey[idx11];
        
        double Bz_interp = w00 * Bz[idx00] + w10 * Bz[idx10] +
 ...
 */
void advance_particles_cpu_simd_aggressive(
    double* x, double* y,
    double* vx, double* vy,
    const double* Ex, const double* Ey, const double* Bz,
    int nx, int ny, double dx, double dy,
    ...
    int species_id)
{
    ...
    for (int chunk_start = 0; chunk_start < n_particles; 
