...
inline int init_mpi(int argc, char** argv, MPIContext& ctx,
    ...
    if (ctx.rank == 0) {
        std::cout << "MPI initialized with " << ctx.size << " processes\n"
                  << "Rank grid: " << ctx.dims[0] << " × " << ctx.dims[1]
                  << std::endl;
    }
    ...
    MPI_Cart_create(MPI_COMM_WORLD, 2, ctx.dims.data(), 
                   ctx.periods.data(), 1, &ctx.comm_2d);
    ...
    if (ctx.rank == 0) {
        std::cout << "MPI Cartesian communicator created successfully\n"
                  ...
                  << "): neighbors L=" << ctx.rank_left 
                  << " R=" << ctx.rank_right
                  << " T=" << ctx.rank_top
                  << " B=" << ctx.rank_bottom << std::endl;
    }
 ...
 */
inline void setup_domain(const MPIContext& ctx, MPIDomain& domain,
 ...
 */
inline void exchange_ghosts_irecv(
    ...
    if (ctx.rank_top != MPI_PROC_NULL) {
        ...
        MPI_Isend(send_buf.data(), nx, MPI_DOUBLE,
                 ctx.rank_top, 1, ctx.comm_2d, &requests[req_idx++]);
        
        // Receive top ghost cells from top neighbor
        MPI_Irecv(domain.ghost_top.data(), nx, MPI_DOUBLE,
                 ctx.rank_top, 2, ctx.comm_2d, &requests[req_idx++]);
    }
    ...
    if (ctx.rank_bottom != MPI_PROC_NULL) {
        ...
        MPI_Isend(send_buf.data(), nx, MPI_DOUBLE,
                 ctx.rank_bottom, 2, ctx.comm_2d, &requests[req_idx++]);
        
        // Receive bottom ghost cells
        MPI_Irecv(domain.ghost_bottom.data(), nx, MPI_DOUBLE,
                 ctx.rank_bottom, 1, ctx.comm_2d, &requests[req_idx++]);
    }
    ...
    if (ctx.rank_left != MPI_PROC_NULL) {
        ...
        MPI_Isend(send_buf.data(), ny, MPI_DOUBLE,
                 ctx.rank_left, 3, ctx.comm_2d, &requests[req_idx++]);
        
        // Receive left ghost cells
        MPI_Irecv(domain.ghost_left.data(), ny, MPI_DOUBLE,
                 ctx.rank_left, 4, ctx.comm_2d, &requests[req_idx++]);
    }
    ...
    if (ctx.rank_right != MPI_PROC_NULL) {
        ...
        MPI_Isend(send_buf.data(), ny, MPI_DOUBLE,
                 ctx.rank_right, 4, ctx.comm_2d, &requests[req_idx++]);
        
        // Receive right ghost cells
        MPI_Irecv(domain.ghost_right.data(), ny, MPI_DOUBLE,
                 ctx.rank_right, 3, ctx.comm_2d, &requests[req_idx++]);
    }
