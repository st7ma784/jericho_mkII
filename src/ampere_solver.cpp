...
double collect_global_current(const double* Jx_local, const double* Jy_local,
    ...
    MPI_Allreduce(Jx_local, Jx_sum.data(), total_points,
                  MPI_DOUBLE, MPI_SUM, mpi_state.cart_comm);
    MPI_Allreduce(Jy_local, Jy_sum.data(), total_points,
                  MPI_DOUBLE, MPI_SUM, mpi_state.cart_comm);
    ...
    double total_current = compute_global_current_magnitude(Jx_global.data(), Jy_global.data(),
                                                           nx, ny, 1.0, 1.0,
                                                           mpi_state, use_device);
...
double compute_global_current_magnitude(const double* Jx, const double* Jy,
    ...
    MPI_Allreduce(&local_j_mag, &global_j_mag, 1, MPI_DOUBLE, MPI_SUM,
                  mpi_state.cart_comm);
