...
double EnergyHistory::conservation_error(int step) const {
    if (step < 0 || step >= (int)total_energy.size()) return 0.0;
...
double EnergyHistory::average_error(int start_step, int end_step) const {
    ...
    if (start_step >= end_step) return 0.0;
 ...
 */
bool validate_energy_conservation(
    const EnergyHistory& history,
    double tolerance,
    int start_step,
    int end_step) {
    
    if (history.total_energy.empty()) return false;
