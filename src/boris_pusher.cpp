...
void advance_particles_boris(ParticleBuffer& particles, const double* Ex, const double* Ey,
    ...
    for (size_t i = 0; i < particles.count; ++i) {
        ...
        double coriolis_factor =
            (config.enable_rotating_frame && config.enable_coriolis) ? 1.0 : 0.0;
        double centrifugal_factor =
            (config.enable_rotating_frame && config.enable_centrifugal) ? 1.0 : 0.0;
        ...
        particles.vx[i] =
            vx_plus + half_qm_dt * ex + centrifugal_factor * half_Omega_dt * Omega * x;
        particles.vy[i] =
            vy_plus + half_qm_dt * ey + centrifugal_factor * half_Omega_dt * Omega * y;
    }
}
