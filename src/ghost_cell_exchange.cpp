 ...
 */
void synchronize_field_boundaries(FieldArrays& fields,
    ...
    MPI_Irecv(recv_north.data(), send_north.size(), MPI_DOUBLE,
             mpi_state.neighbors[0], 0, mpi_state.cart_comm, 
             &mpi_state.exchange_state.requests[0]);
    
    MPI_Irecv(recv_south.data(), send_south.size(), MPI_DOUBLE,
             mpi_state.neighbors[1], 1, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[1]);
    
    MPI_Irecv(recv_east.data(), send_east.size(), MPI_DOUBLE,
             mpi_state.neighbors[2], 2, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[2]);
    
    MPI_Irecv(recv_west.data(), send_west.size(), MPI_DOUBLE,
             mpi_state.neighbors[3], 3, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[3]);
    
    // Post non-blocking sends to all 4 neighbors
    MPI_Isend(send_north.data(), send_north.size(), MPI_DOUBLE,
             mpi_state.neighbors[0], 1, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[4]);
    
    MPI_Isend(send_south.data(), send_south.size(), MPI_DOUBLE,
             mpi_state.neighbors[1], 0, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[5]);
    
    MPI_Isend(send_east.data(), send_east.size(), MPI_DOUBLE,
             mpi_state.neighbors[2], 3, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[6]);
    
    MPI_Isend(send_west.data(), send_west.size(), MPI_DOUBLE,
             mpi_state.neighbors[3], 2, mpi_state.cart_comm,
             &mpi_state.exchange_state.requests[7]);
 ...
 */
void synchronize_all_fields(FieldArrays& fields,
                           struct MPIDomainState& mpi_state) {
 ...
 */
bool verify_boundary_continuity(const FieldArrays& fields,
                               struct MPIDomainState& mpi_state,
    ...
    device_memcpy(&local_value, &field_ptr[corner_idx], 
                  sizeof(double), "device-to-host");
 ...
 */
void print_boundary_stats(const FieldArrays& fields,
                         const double* field_ptr,
    ...
    printf("  North: min=%.3e, max=%.3e, mean=%.3e\n",
           north_min, north_max, north_sum / north.size());
}
