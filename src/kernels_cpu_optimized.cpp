 ...
 */
void particle_to_grid_atomic(const double* x, const double* y, const double* vx, const double* vy,
                             double* rho, double* Jx, double* Jy, int nx, int ny, double dx,
                             double dy, double x_min, double y_min, double q, int n_particles,
                             int species_id) {
 ...
 */
void particle_to_grid_tiled(const double* x, const double* y, const double* vx, const double* vy,
                            double* rho, double* Jx, double* Jy, int nx, int ny, double dx,
                            double dy, double x_min, double y_min, double q, int n_particles,
                            int species_id) {
        ...
        for (int ty = 0; ty < n_tiles_y; ty++) {
            ...
            for (int p = 0; p < n_particles; p++) {
                ...
                if (tile_x == tx && tile_y == ty) {
                    ...
                    if (i >= tx * TILE_SIZE && i < (tx + 1) * TILE_SIZE && j >= ty * TILE_SIZE &&
                        j < (ty + 1) * TILE_SIZE) {
 ...
 */
void advance_particles_simd(double* x, double* y, double* vx, double* vy, const double* Ex,
                            const double* Ey, const double* Bz, int nx, int ny, double dx,
                            double dy, double x_min, double y_min, double dt,
                            const double* qm_by_type, int n_particles, int species_id) {
    const double dt_half = dt * 0.5;
    const double qm = qm_by_type[species_id];

// Vectorization hints for compiler
#pragma omp parallel for simd collapse(1) schedule(static) aligned(x, y, vx, vy : 32) reduction(   \
        + : x[0 : n_particles], y[0 : n_particles], vx[0 : n_particles], vy[0 : n_particles])
    for (int p = 0; p < n_particles; p++) {
        ...
        double Ex_interp = w00 * Ex[j * nx + i] + w10 * Ex[j * nx + i1] + w01 * Ex[j1 * nx + i] +
                           w11 * Ex[j1 * nx + i1];

        double Ey_interp = w00 * Ey[j * nx + i] + w10 * Ey[j * nx + i1] + w01 * Ey[j1 * nx + i] +
                           w11 * Ey[j1 * nx + i1];

        double Bz_interp = w00 * Bz[j * nx + i] + w10 * Bz[j * nx + i1] + w01 * Bz[j1 * nx + i] +
                           w11 * Bz[j1 * nx + i1];
...
void particle_to_grid_cpu(const double* x, const double* y, const double* vx, const double* vy,
    ...
    particle_to_grid_atomic(x, y, vx, vy, rho, Jx, Jy, nx, ny, dx, dy, x_min, y_min, q, n_particles,
                            0 /* species_id */);
}
...
void advance_particles_cpu(double* x, double* y, double* vx, double* vy, const double* Ex,
    ...
    advance_particles_simd(x, y, vx, vy, Ex, Ey, Bz, nx, ny, dx, dy, x_min, y_min, dt, qm_by_type,
                           n_particles, species_id);
}
