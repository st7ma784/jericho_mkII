...
void initialize_harris_sheet(SimulationParams& params,
                             FieldArrays& fields,
                             ParticleBuffer& ions,
    ...
    double di = c / sqrt(params.n0 * constants::e * constants::e /
...
void compute_diagnostics(const FieldArrays& fields,
                        const ParticleBuffer& ions,
                        const ParticleBuffer& electrons,
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        params.particles_per_cell = config.species.empty() ? 1 : config.species[0].particles_per_cell;
    ...
    if (params.npx <= 0) params.npx = 1;
    if (params.npy <= 0) params.npy = 1;
    ...
    MPIManager mpi(&argc, &argv,
                   params.npx, params.npy,
                   params.nx_global, params.ny_global,
                   0.0, Lx_phys, 0.0, Ly_phys,
    ...
    FieldArrays fields(mpi.nx_local, mpi.ny_local, params.nghost,
                       dx, dy, mpi.x_min_local, mpi.y_min_local,
                       0);  // Device ID
    ...
    initialize_harris_sheet(params, fields, ions, electrons,
                           mpi.x_min_local, mpi.y_min_local);

    // Initialize particles uniformly
    if (rank == 0) std::cout << "Initializing particles..." << std::endl;
    ...
    if (rank == 0) std::cout << "  v_ti = " << v_ti << ", v_te = " << v_te << ", weight = " << weight << std::endl;
    if (rank == 0) std::cout << "  Initializing ions..." << std::endl;

    initialize_uniform(ions,
                      mpi.x_min_local, mpi.x_max_local,
                      mpi.y_min_local, mpi.y_max_local,
                      params.particles_per_cell,
                      mpi.nx_local, mpi.ny_local,
                      0, weight, 0.0, 0.0, v_ti, 42 + rank);

    if (rank == 0) std::cout << "  Ions initialized: " << ions.count << " particles" << std::endl;
    if (rank == 0) std::cout << "  Initializing electrons..." << std::endl;
    ...
    initialize_uniform(electrons,
                      ...
                      params.particles_per_cell,
                      mpi.nx_local, mpi.ny_local,
                      1, weight, 0.0, 0.0, v_te, 1337 + rank);
    ...
    for (int step = 0; step < params.max_steps; step++) {
        ...
        if (rank == 0 && step == 1) std::cout << "  Advancing ions..." << std::endl;
        cuda::advance_particles_cpu(ions.x, ions.y, ions.vx, ions.vy,
                                   fields.Ex, fields.Ey, fields.Bz,
                                   fields.nx, fields.ny,
                                   fields.dx, fields.dy,
                                   fields.x_min, fields.y_min,
                                   dt_phys, qm_by_type.data(),
                                   ions.count, 0);  // species_id=0 for ions

        if (rank == 0 && step == 1) std::cout << "  Advancing electrons..." << std::endl;
        cuda::advance_particles_cpu(electrons.x, electrons.y, electrons.vx, electrons.vy,
                                   fields.Ex, fields.Ey, fields.Bz,
                                   fields.nx, fields.ny,
                                   fields.dx, fields.dy,
                                   fields.x_min, fields.y_min,
                                   dt_phys, qm_by_type.data(),
                                   electrons.count, 1);  // species_id=1 for electrons

        // ---------------------------------------------------------------------
        // Step 2: Particle-to-grid (deposit charge and current)
        // ---------------------------------------------------------------------

        if (rank == 0 && step == 1) std::cout << "  Particle-to-grid..." << std::endl;
        fields.zero_particle_quantities();

        // Deposit ions
        cuda::particle_to_grid_cpu(ions.x, ions.y, ions.vx, ions.vy,
                                  fields.charge_density,
                                  fields.Jx, fields.Jy,
                                  fields.nx, fields.ny,
                                  fields.dx, fields.dy,
                                  fields.x_min, fields.y_min,
                                  constants::e, ions.weight[0],  // charge and weight
                                  ions.count);
