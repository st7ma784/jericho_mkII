...
MPIDomainState initialize_mpi_topology(int rank, int size, 
    ...
    while (size % g_npx != 0) g_npx--;
    g_npy = size / g_npx;
    
    if (rank == 0) {
        std::cout << "MPI Topology: " << g_npx << " x " << g_npy 
                  << " (" << size << " ranks total)" << std::endl;
    }
 ...
 */
void start_ghost_cell_exchange_nonblocking(FieldArrays& fields, 
                                           MPIDomainState& mpi_state) {
    ...
    MPI_Irecv(nbx.recv_buffers[0].data(), n_north, MPI_DOUBLE,
             mpi_state.neighbors[0], 0, mpi_state.cart_comm, &nbx.requests[0]);
    MPI_Irecv(nbx.recv_buffers[1].data(), n_south, MPI_DOUBLE,
             mpi_state.neighbors[1], 1, mpi_state.cart_comm, &nbx.requests[1]);
    MPI_Irecv(nbx.recv_buffers[2].data(), n_east, MPI_DOUBLE,
             mpi_state.neighbors[2], 2, mpi_state.cart_comm, &nbx.requests[2]);
    MPI_Irecv(nbx.recv_buffers[3].data(), n_west, MPI_DOUBLE,
             mpi_state.neighbors[3], 3, mpi_state.cart_comm, &nbx.requests[3]);
    
    // Post sends for all 4 directions
    MPI_Isend(nbx.send_buffers[0].data(), n_north, MPI_DOUBLE,
             mpi_state.neighbors[0], 1, mpi_state.cart_comm, &nbx.requests[4]);
    MPI_Isend(nbx.send_buffers[1].data(), n_south, MPI_DOUBLE,
             mpi_state.neighbors[1], 0, mpi_state.cart_comm, &nbx.requests[5]);
    MPI_Isend(nbx.send_buffers[2].data(), n_east, MPI_DOUBLE,
             mpi_state.neighbors[2], 3, mpi_state.cart_comm, &nbx.requests[6]);
    MPI_Isend(nbx.send_buffers[3].data(), n_west, MPI_DOUBLE,
             mpi_state.neighbors[3], 2, mpi_state.cart_comm, &nbx.requests[7]);
 ...
 */
void accumulate_local_diagnostics(double E_local, int N_local,
                                  MPIDomainState& mpi_state) {
 ...
 */
bool flush_batched_diagnostics(double& E_global, int& N_global,
                               MPIDomainState& mpi_state) {
    if (mpi_state.diagnostic_step_counter >= mpi_state.DIAGNOSTIC_INTERVAL) {
        ...
        MPI_Allreduce(MPI_IN_PLACE, &E_global, 1, MPI_DOUBLE,
                     MPI_SUM, mpi_state.cart_comm);
        MPI_Allreduce(MPI_IN_PLACE, &N_global, 1, MPI_INT,
                     MPI_SUM, mpi_state.cart_comm);
...
void gather_global_diagnostics_legacy(double& E_total, int& N_particles,
    ...
    MPI_Allreduce(MPI_IN_PLACE, &E_total, 1, MPI_DOUBLE,
                 MPI_SUM, mpi_state.cart_comm);
    MPI_Allreduce(MPI_IN_PLACE, &N_particles, 1, MPI_INT,
                 MPI_SUM, mpi_state.cart_comm);
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        FieldArrays fields(mpi_state.nx_local, mpi_state.ny_local, 1,
                          mpi_state.x_min_local, mpi_state.y_min_local,
        ...
        if (rank == 0) {
            ...
            std::cout << "  Expected dispersion: ω = c|k| = " << (SPEED_OF_LIGHT * WAVE_NUMBER) << " rad/s" << std::endl;
        }
        ...
        // Ions: H+ protons
        initialize_uniform(ions,
                          0.0, mpi_state.nx_local * fields.dx,
                          0.0, mpi_state.ny_local * fields.dy,
                          particles_per_cell, mpi_state.nx_local, mpi_state.ny_local,
                          1,        // type = 1 for ions
                          1.0,      // weight = 1.0
        ...
        if (rank == 0) {
            ...
            std::cout << "  Local grid: " << mpi_state.nx_local << " x " 
                     << mpi_state.ny_local << " per rank" << std::endl;
            std::cout << "  Particles per rank (ions): " << ions.count << std::endl;
            std::cout << "  Particles per rank (electrons): " << electrons.count << std::endl;
            std::cout << "  Total particles: " << (ions.count + electrons.count) * size << std::endl;
        ...
        for (int step = 0; step < params.max_steps; step++) {
            ...
            apply_field_boundary_conditions(fields.Ex, fields.Ey,
            ...
            zero_current_density(fields.Jx, fields.Jy,
                                fields.nx, fields.ny, false);  // false = CPU mode
            ...
            compute_current_density(
                ...
                fields.Jx, fields.Jy,
                fields.nx, fields.ny,
                fields.dx, fields.dy, false);  // false = CPU mode
            ...
            compute_current_density(
                electrons.x, electrons.y,
                electrons.vx, electrons.vy,
                electrons_q.data(),
                electrons.count,
                Jx_electrons.data(), Jy_electrons.data(),
                fields.nx, fields.ny,
                fields.dx, fields.dy, false);  // false = CPU mode
            ...
            advance_magnetic_field_faraday(fields.Bz,
                                          fields.Ex, fields.Ey,
                                          fields.nx, fields.ny,
                                          fields.dx, fields.dy,
                                          params.dt,
            ...
            // PHASE 12 TEST 3: ENABLED for EM wave propagation
            advance_electric_field_ampere(fields.Ex, fields.Ey,
                                         fields.Bz,
                                         fields.Jx, fields.Jy,
                                         fields.nx, fields.ny,
                                         fields.dx, fields.dy,
            ...
            advance_particles_boris(electrons,
                                   fields.Ex, fields.Ey, fields.Bz,
                                   fields.nx, fields.ny,
                                   fields.dx, fields.dy,
                                   params.dt,
            ...
            if ((step % 10) == 0) {
                ...
                for (size_t i = 0; i < ions.count; ++i) {
                    if (std::isnan(ions.vx[i]) || std::isinf(ions.vx[i])) nan_ions++;
                    max_vx_ions = std::max(max_vx_ions, std::abs(ions.vx[i]));
                    max_vy_ions = std::max(max_vy_ions, std::abs(ions.vy[i]));
                }
                for (size_t i = 0; i < electrons.count; ++i) {
                    if (std::isnan(electrons.vx[i]) || std::isinf(electrons.vx[i])) nan_elecs++;
                ...
                if (rank == 0 && (nan_ions > 0 || nan_elecs > 0 || step < 20)) {
                    std::cerr << "Step " << step << ": max_vx_ions=" << max_vx_ions 
                              << ", max_vy_ions=" << max_vy_ions
                              << ", nan_ions=" << nan_ions
                              << ", max_vx_elecs=" << max_vx_elecs
                              << ", max_vy_elecs=" << max_vy_elecs
                              << ", nan_elecs=" << nan_elecs << std::endl;
                }
                ...
                if (std::isnan(ke_electrons) || std::isinf(ke_electrons)) {
                    if (rank == 0) {
                        std::cerr << "WARNING: ke_electrons is " << (std::isnan(ke_electrons) ? "NaN" : "inf")
                                  << " at step " << step << " on rank " << rank << std::endl;
                    }
                ...
                MPI_Allreduce(MPI_IN_PLACE, &ke_ions_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                MPI_Allreduce(MPI_IN_PLACE, &ke_electrons_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        ...
        if (rank == 0) {
            ...
            std::cout << "Time per step: " << (loop_time / params.max_steps) * 1000 << " ms" << std::endl;
            
            double particles_per_sec = (n_particles_local * 2 * size) * params.max_steps / loop_time / 1e6;
            std::cout << "Performance: " << particles_per_sec << " million particle pushes/sec" << std::endl;
            ...
            if (!energy_history.total_energy.empty()) {
                ...
                std::cout << "Initial total energy: " << std::scientific << E_initial << " J" << std::endl;
                std::cout << "Final total energy: " << E_final << " J" << std::endl;
                std::cout << "Energy change: " << E_change << " J" << std::endl;
                std::cout << "Relative error: " << std::fixed << (E_relative_error * 100.0) << " %" << std::endl;
                
                // Check conservation criterion (±1%)
                double conservation_tolerance = 0.01;  // 1%
                bool is_conserved = validate_energy_conservation(
                    energy_history, conservation_tolerance, 0, -1);
                ...
                if (is_conserved) {
                    ...
                    std::cout << "  Average relative error: " << (avg_error * 100.0) << " %" << std::endl;
                }
                ...
                if (!energy_history.ke_ions.empty()) {
                    ...
                    std::cout << "  Ions: " << KE_ions_initial << " → " << KE_ions_final << " J" << std::endl;
                    std::cout << "  Electrons: " << KE_elec_initial << " → " << KE_elec_final << " J" << std::endl;
                }
            }
            std::cout << std::endl;
        }
