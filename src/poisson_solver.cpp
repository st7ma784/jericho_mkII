...
double collect_global_charge_density(const FieldArrays& fields,
                                     ...
                                     const MPIDomainState& mpi_state, bool use_device) {
    ...
    if (use_device) {
        ...
        std::memcpy(local_charge.data(), fields.charge_density, total_points * sizeof(double));
    } else {
        std::memcpy(local_charge.data(), fields.charge_density, total_points * sizeof(double));
    }

    // Global reduction: sum all local charge densities
    MPI_Allreduce(local_charge.data(), global_charge_sum.data(), total_points, MPI_DOUBLE, MPI_SUM,
                  mpi_state.cart_comm);
...
double compute_global_charge(const double* charge_density, int nx, int ny, double dx, double dy,
    ...
    MPI_Allreduce(&local_charge, &global_charge, 1, MPI_DOUBLE, MPI_SUM, mpi_state.cart_comm);
...
int solve_poisson_equation(const double* charge_density, double* potential, int nx, int ny,
                           double dx, double dy, const PoissonConfig& config, bool use_device) {
    ...
    if (config.use_fft) {
        ...
        return solve_poisson_sor(charge_density, potential, nx, ny, dx, dy, config.omega,
                                 config.max_iterations, config.convergence_tol, use_device);
    }
}
...
int solve_poisson_sor(const double* rhs, double* solution, int nx, int ny, double dx, double dy,
                      double omega, int max_iter, double tol, bool use_device) {
    ...
    for (iteration = 0; iteration < max_iter; ++iteration) {
        ...
        for (int iy = 1; iy < ny - 1; ++iy) {
            for (int ix = 1; ix < nx - 1; ++ix) {
                ...
                double phi_new = coeff_x * (solution[idx_xp] + solution[idx_xm]) +
                                 coeff_y * (solution[idx_yp] + solution[idx_ym]) - coeff * rhs[idx];
        ...
        for (int iy = 1; iy < ny - 1; ++iy) {
            for (int ix = 1; ix < nx - 1; ++ix) {
                ...
                double laplacian =
                    (solution[idx_xp] + solution[idx_xm] - 2.0 * solution[idx]) / dx2 +
...
int solve_poisson_fft(const double* rhs, double* solution, int nx, int ny, double dx, double dy,
    ...
    return solve_poisson_sor(rhs, solution, nx, ny, dx, dy, 1.9, 5000, 1e-8, use_device);
}
...
void compute_electric_field(const double* potential, double* Ex, double* Ey, int nx, int ny,
                            double dx, double dy, bool use_device) {
...
void apply_field_boundary_conditions(double* Ex, double* Ey, const double* Ex_bg,
                                     const double* Ey_bg, int nx, int ny, const char* bc_type,
                                     bool use_device) {
...
double verify_poisson_solution(const double* potential, const double* charge_density, int nx,
                               int ny, double dx, double dy, bool use_device) {
    ...
    for (int iy = 1; iy < ny - 1; ++iy) {
        for (int ix = 1; ix < nx - 1; ++ix) {
            ...
            double laplacian =
                (potential[idx_xp] + potential[idx_xm] - 2.0 * potential[idx]) / dx2 +
...
void print_poisson_stats(const FieldArrays& fields, const MPIDomainState& mpi_state,
                         int iteration_count, double convergence_error) {
    ...
    std::cout << "  Grid spacing:              dx=" << fields.dx << " m, dy=" << fields.dy << " m"
              << std::endl;
    std::cout << "  Ranks: " << mpi_state.rank << "/"
              << (mpi_state.neighbors[0] + mpi_state.neighbors[1] + mpi_state.neighbors[2] +
                  mpi_state.neighbors[3])
              << std::endl;
